...
namespace EmptyProject_Work_01.Controllers
{
    ...
    public class OnlinejobsiteDataController : Controller
    {

        JobDbContext db;
        public OnlinejobsiteDataController(JobDbContext db) { this.db = db; }
        ...
        [HttpGet]
        public IActionResult OnlinejobsiteWithAdvertisement()
        {
            var data = db.OnlineJobSites.Include(x => x.JobAdvertisements).Include(x => x.RegisterApplicants).ToList();

            return Json(data);

        }
        [Route("custom/OnlinejobsiteData/{id}")]
        public IActionResult OnlinejobsiteWithAdvertisement(int id)
        {
